...
public class QueryBqlActivity extends AppCompatActivity {
     ...
     * @param sql
     */
    public void queryBql(View view, String sql) {
     ...
     */
    private void queryStatisticSql(View view) {
                ...
                if (e == null) {
                    if (ary != null) {
                        try {
                            ...
                            Snackbar.make(view, nickname + "-" + sum, Snackbar.LENGTH_LONG).show();
                        } catch (JSONException ex) {
                            ex.printStackTrace();
                        }
                    } else {
                        Snackbar.make(view, "查询成功，无数据", Snackbar.LENGTH_LONG).show();
                    }
                } else {
                    Snackbar.make(view, e.getMessage(), Snackbar.LENGTH_LONG).show();
                }
     ...
     */
    private void queryPlaceholderSql1(View view) {
        BmobQuery<User> query = new BmobQuery<>();
                ...
                if (e == null) {
                    ...
                    if (list != null && list.size() > 0) {
                        for (int i = 0; i < list.size(); i++) {
                            ...
                            Snackbar.make(view, user.getNickname() + "-" + user.getScore(), Snackbar.LENGTH_LONG).show();
                        }
                    } else {
                        Snackbar.make(view, "查询成功，无数据", Snackbar.LENGTH_LONG).show();
                    }
     ...
     * select * from _User where createdAt > date( ?) and address near geopoint ( ?,?)
     */
    private void queryPlaceholderSql2(View view) {
        String sql = "select * from _User where createdAt > date( ?) and address near geopoint ( ?,?)";
        //等同于下面的查询方法
    ...
    public void onQuerySql(View view) {
        String sql = "select * from _User";
        queryBql(view, sql);


        queryStatisticSql(view);

        queryPlaceholderSql1(view);

        queryPlaceholderSql2(view);
    }
}
